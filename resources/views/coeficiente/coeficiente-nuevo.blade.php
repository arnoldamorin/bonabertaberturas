...
@section('scripts')
<script>
    globalId = '<?php echo isset($coeficiente->idcoeficiente) && $coeficiente->idcoeficiente > 0 ? $coeficiente->idcoeficiente : 0; ?>';
    <?php $globalId = isset($coeficiente->idcoeficiente) ? $coeficiente->idcoeficiente : "0"; ?>
        ...
        <form id="form1" method="POST">
            <div class="row">
                ...
                <div class="form-group col-lg-6">
                    <label>Nombre: *</label>
                    <input type="text" id="txtNombre" name="txtNombre" class="form-control" value="{{ $coeficiente->nombre or '' }}" required>
                </div>                
                <div class="form-group col-lg-6">
                    <label>Valor:</label>
                    <input type="text" id="txtValor" name="txtValor" class="form-control" value="{{$coeficiente->valor or ''}}">
                </div>                
            </div>			
        </form>
</div>
...
<script>
    ...
    function eliminar() {       
        $.ajax({
            type: "GET",
            url: "{{ asset('admin/coeficiente/eliminar') }}",
            data: { id:globalId },
            async: true,
            dataType: "json",
